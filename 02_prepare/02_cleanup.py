...
import os, shutil
from _convert_xml_to_df import created_merged_df
...
def created_merged_df():
    print("Merging XML files into DataFrame")
    global root_folder
    root_folder='../03_clones/duplicates/'
    ...
    os.makedirs('../03_clones/duplicates/subelem_final', exist_ok=True)
    ...
    fs = os.listdir('../03_clones/duplicates/subelem_final')
    merged_df = pd.concat([pickle.load(open('../03_clones/duplicates/subelem_final/'+x, 'rb')) for x in fs])
    
    print("Persisting DataFrame into p file")
    pickle.dump(merged_df, open('../03_clones/duplicates/clones.p', 'wb'))
    
    shutil.rmtree('../03_clones/duplicates/subelem_final', ignore_errors=True)
    
def copyFiles():
    print("Copying files")
    copy_tree('../03_clones/duplicates/final', '../03_clones/rq1-rq2')
    shutil.copy('../03_clones/duplicates/clones.p', '../03_clones/rq1-rq2')
    
if __name__=="__main__":
    os.chdir('../03_clones')
    remove_all_duplicates('macro')
    created_merged_df()
    copyFiles()