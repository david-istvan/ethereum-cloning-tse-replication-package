...
def prepareObservation3():
    allcontracts = pd.read_json('./fulldata/authorinfo.json').transpose()
    ...
    allcontracts['nclones'] =  allcontracts.apply(lambda row: getNClones(row.name), axis=1)
    
    allcontracts = allcontracts.sort_values(by=['time'], ascending=True)
    
    allcontracts.to_pickle('./data/observation3data.p')

def prepareObservation3v2():
    ...
    allcontracts.to_pickle('./data/observation3data-detailed.p')
...
def prepareObservation10():
    ...
    authorDf.to_pickle('./data/observation10data.p')
